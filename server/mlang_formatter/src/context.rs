...
pub struct MFormatContext {
    ...
    comments: Rc<MComments>,

    /// Stores the formatted content of one function body.
    ///
    /// Used during formatting of call arguments where function expressions
    ...
    /// where the algorithm is quadratic, in case the function contains another
    ...
    cached_function_body: Option<(AnyMFunctionBody, FormatElement)>,

    source_map: Option<TransformSourceMap>,
}

impl MFormatContext {
    pub fn new(options: MFormatOptions, comments: MComments) -> Self {
        Self {
            options,
    ...
    ///
    /// See [MFormatContext::cached_function_body] for more in depth documentation.
    pub(crate) fn get_cached_function_body(
        &self,
        body: &AnyMFunctionBody,
...
impl FormatContext for MFormatContext {
    type Options = MFormatOptions;
...
impl CstFormatContext for MFormatContext {
    type Language = MLanguage;
    type Style = MCommentStyle;
    type CommentRule = FormatMLeadingComment;

    fn comments(&self) -> &MComments {
        &self.comments
    }
}
...
impl MFormatOptions {
    pub fn new(source_type: MFileSource) -> Self {
        Self {
            source_type,
...
impl FormatOptions for MFormatOptions {
...
impl fmt::Display for MFormatOptions {
